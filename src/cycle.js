...
let staticServer;
async function getStaticServer(host = 'localhost') {
  if (!staticServer) {
    let getStatic = (() => {
      let statics = {}
      return docroot => {
    ...
    staticServer = http.createServer((req, resp) => {
    ...
    await new Promise((res, rej) => {
      staticServer.once('listening', res);
...
export default class Cycle extends EventEmitter {
  ...
  serve(opts) {
    return co.call(this, function* () {
      ...
      opts = _.extend({
        ...
        strategy: 'replace',
        host: this._proxy.address().address
      }, opts)
      let { docroot, path, strategy, host } = opts
      ...
      let staticResp = yield new Promise(async (resolve, reject) => {
        let addr = (await getStaticServer(host)).address()
        http.get({
   ...
   */
  _sendToServer() {
    let upstreamProxy = this._request.upstreamProxy === undefined
      ? this._proxy._upstreamProxy
      : this._request.upstreamProxy
