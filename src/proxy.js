    ...
    constructor(opts = {}) {
        ...
        this._server = createServer((fromClient, toClient) => {
            ...
            co.call(this, function* () {
                ...
                try { yield this._runIntercepts('response-sent', cycle) } catch (ex) { this._emitError(ex, 'response-sent') }
            }).catch(ex => {
                this.emit('error', ex)
                this.emit('log', {
                    level: 'error',
        ...
        if (opts.certAuthority) {
            ...
            let { key, cert } = opts.certAuthority, spoofer = new SNISpoofer(key, cert), SNICallback = spoofer.callback(), cxnEstablished = new Buffer(`HTTP/1.1 200 Connection Established\r\n\r\n`, 'ascii')
